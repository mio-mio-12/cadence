...
int main(){
 if(!glfwInit())return 2;glfwWindowHint(GLFW_VISIBLE,GLFW_FALSE);auto*w=glfwCreateWindow(960,540,"v141 checks",nullptr,nullptr);if(!w)return 3;glfwMakeContextCurrent(w);ImGui::CreateContext();ImGui::GetIO().IniFilename=nullptr;
 ...
 const auto out=std::filesystem::path("diagnostics/v141");std::filesystem::create_directories(out);std::ofstream report(out/"transitions.txt");
 ...
 for(auto hand:{"viewmodel_SWAT_Male_hands","viewmodel_SWAT_Female_SWAT-Sniper_hands","c_usa_mp_isa_smg_viewhands_LOD0"})for(auto weapon:{"viewmodel_pistol_DesertEagle-Dual","viewmodel_pistol_DesertEagle","viewmodel_knife_M-9_Dual"}){
  ...
  if(std::string(weapon)=="viewmodel_pistol_DesertEagle"&&std::string(hand)=="viewmodel_SWAT_Male_hands"){
   ...
   a.renderer.setWeaponMetalnessFromDiffuse(true,.2f,.8f,1);CHECK(render(baseline,"metalness_diffuse"));std::vector<uint8_t> diffuse,inverted,equal;CHECK(a.renderer.readColorRgba(diffuse,error));CHECK(diffuse!=metal);
   a.renderer.setWeaponMetalnessFromDiffuse(true,.8f,.2f,1);CHECK(render(baseline,"metalness_inverted"));CHECK(a.renderer.readColorRgba(inverted,error));CHECK(inverted!=diffuse);
   a.renderer.setWeaponMetalnessFromDiffuse(true,.5f,.5f,1);CHECK(render(baseline,"metalness_equal"));CHECK(a.renderer.readColorRgba(equal,error));
 ...
 for(bool forceHip:{false,true})for(auto hand:{"codm_viewhands_c_m_ghost_Default","c_usa_mp_isa_smg_viewhands_LOD0"})for(auto weapon:{"viewmodel_shotty_m1887_Default","viewmodel_sniper_locus_scuba"}){
  reset();a.selectedBaseAsset=idx(hand);const auto wi=idx(weapon);CHECK(wi<a.assetCatalog.entries.size());equipViewWeapon(a,wi);reset();if(forceHip)a.weaponProfile.animations["ads_rechamber"]=a.weaponProfile.animations.at("rechamber");auto idle=findViewmodelClip(a,"idle"),up=findViewmodelClip(a,"ads_up"),down=findViewmodelClip(a,"ads_down"),bolt=findViewmodelClip(a,"ads_rechamber");CHECK(idle&&up&&down&&bolt);a.animationIndex=*idle;a.animationFrame=0;
  CHECK(startViewmodelAimClip(a,*up));a.viewmodelAdsBaseFrame=a.scene.animations[*up].durationFrames;a.viewmodelAdsTransitionElapsed=a.weaponTiming.adsIn;a.gameplayAds=true;a.gameplayRechamber=true;a.gameplayAction=scene::ActionRole::Reload;triggerGameplayAction(a);CHECK(a.actionOverlay);const auto action=a.actionAnimationIndex;CHECK(action==*bolt);CHECK(cadence::codm_actions::hipBolt(a.scene,action)==forceHip);a.actionFrame=a.scene.animations[action].durationFrames*.4f;a.actionElapsed=authoredClipDuration(a,action)*.4f;a.interruptPoseDuration=0;a.transitioning=false;
  ...
  for(int f=0;f<=4;++f){a.viewmodelAdsBaseFrame=a.scene.animations[*down].durationFrames*f/4.f;auto p=evaluateCurrentPose(a);for(auto&m:p)for(auto v:m.v)CHECK(std::isfinite(v));last=p;if(forceHip){const auto anchor=a.scene.skeleton.boneByName.at("Muzzle_point");const auto source=a.scene.samplePose(action,frame);const auto inv=scene::inverseAffine(p[anchor]),sourceInv=scene::inverseAffine(source[anchor]);for(const auto name:{"b_RightHand","b_LeftHand"}){auto it=a.scene.skeleton.boneByName.find(name);CHECK(it!=a.scene.skeleton.boneByName.end());auto relative=inv*p[it->second],original=sourceInv*source[it->second];for(int k=0;k<16;++k)CHECK(std::abs(relative.v[k]-original.v[k])<.02f);}}CHECK(render(p,std::string(weapon)+"_"+hand+(forceHip?"_hipbolt_release_":"_adsbolt_release_")+std::to_string(f)));}
  ...
  a.actionElapsed=duration>0?duration:authoredClipDuration(a,action);a.actionFrame=a.scene.animations[action].durationFrames;auto ended=evaluateCurrentPose(a);a.actionActive=a.actionOverlay=false;CHECK(difference(ended,evaluateCurrentPose(a))<.002f);
  report<<"PASS "<<weapon<<" / "<<hand<<" hip="<<forceHip<<" ADS release during bolt, entry/exit continuity and unchanged action clock\n";report.flush();
 }

 for(bool hip:{false,true}){
  reset();a.selectedBaseAsset=idx("c_usa_mp_isa_smg_viewhands_LOD0");equipViewWeapon(a,idx("viewmodel_shotty_m1887_Default"));reset();
  if(hip)a.weaponProfile.animations["ads_rechamber"]=a.weaponProfile.animations.at("rechamber");
  const auto idle=findViewmodelClip(a,"idle"),up=findViewmodelClip(a,"ads_up");CHECK(idle&&up);a.animationIndex=*idle;a.animationFrame=0;
  a.gameplayRechamber=true;a.gameplayAction=scene::ActionRole::Reload;triggerGameplayAction(a);a.actionElapsed=.2f;a.actionFrame=6;
  const auto bolt=a.actionAnimationIndex;const auto before=evaluateCurrentPose(a);a.gameplayAds=true;a.gameplayAction=scene::ActionRole::Aim;triggerGameplayAction(a);
  CHECK(a.gameplayRechamber&&a.activeAction==scene::ActionRole::Reload&&a.actionAnimationIndex==bolt&&a.actionElapsed==.2f&&a.actionFrame==6);
  CHECK(difference(before,evaluateCurrentPose(a))<.01f);
  a.viewmodelAdsBaseFrame=a.scene.animations[*up].durationFrames;a.viewmodelAdsTransitionElapsed=a.weaponTiming.adsIn;a.interruptPoseElapsed=a.interruptPoseDuration;
  a.actionElapsed=a.actionDurationOverride-.001f;const auto endBefore=evaluateCurrentPose(a);updateGameplay(a,.002f);
  CHECK(!a.gameplayRechamber&&a.activeAction==scene::ActionRole::Aim&&a.viewmodelAdsEngaged);
  CHECK(difference(endBefore,evaluateCurrentPose(a))<.05f);
  report<<"PASS live CODM bolt -> ADS entry and held ADS completion hip="<<hip<<"\\n";report.flush();
 }

 {
  const auto setup=currentAttachmentSetup(a);CHECK(weapon::save(setup,out/"attachment_test.iwweapon",error));weapon::Profile loaded;CHECK(weapon::load(out/"attachment_test.iwweapon",loaded,error));CHECK(applyAttachmentSetup(a,loaded));
  const auto count=a.scene.meshes.size();auto missing=loaded;missing.rigModels={"missing_attachment.cast"};CHECK(!applyAttachmentSetup(a,missing));CHECK(a.scene.meshes.size()==count);
  gameplay::view::CameraControls controls;CHECK(controls.zoomInDuration==0&&controls.zoomOutDuration==0);CHECK(gameplay::view::zoomDuration(controls.zoomInDuration,.32f)==.32f);controls.zoomOutDuration=.74f;std::stringstream stream;stream<<controls;gameplay::view::CameraControls restored;stream>>restored;CHECK(restored.zoomInDuration==0&&std::abs(restored.zoomOutDuration-.74f)<1e-6f);
  report<<"PASS setup roundtrip, failed-load preservation, default clip-matching zoom and independent override persistence\n";
 }
 a.renderer.shutdown();state.reset();ImGui::DestroyContext();glfwDestroyWindow(w);glfwTerminate();std::cout<<"v141 transitions passed\n";
}
